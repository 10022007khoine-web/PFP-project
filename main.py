        ...
        try:
            if pick == 1:
                ...
                if checkid == student_id:
                    ...
                    grade_FE = dis.read_float("Enter grade FE (ex: 6.7 or "-" if not yet graded): ")
                    grade_RE = dis.read_float("Enter grade RE (ex: 3.6 or "-" if not yet graded): ")
                    res = Result(student_id, course_id, course_name, semester, credits, grade_FE, grade_RE)
                    result_manager.add_result(res)
                else:
                ...
                if checkid == student_id and checkcourseid == course_id:
                    grade_FE = dis.read_float("Enter new grade FE (ex: 6.7 or '-' if not yet graded): ")
                    grade_RE = dis.read_float("Enter new grade RE (ex: 3.6 or '-' if not yet graded): ")
                    result_manager.update_result(student_id, course_id, grade_fe=grade_FE, grade_re=grade_RE)
                else:
                ...
                if filter_choice == 1:
                    ...
                    sorted_results = result_manager.sort_by_status_descending()
                    ...
                    sorted_results = sorted(result_manager.results, key=lambda r: r.grade_fe, reverse=True)
                ...
                result_manager.display_passed_student()
